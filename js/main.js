/* PUBLIC UI CONTROLLER — group logic lives in js/grupos.js */
"use strict";
...
const tournamentStages = [
  { name: "FASE DE GRUPOS", note: "32 equipos · 4 grupos de 8 · 2 partidos por equipo" },
  { name: "TOP 16", note: "Clasifican los 4 mejores de cada grupo" },
  { name: "OCTAVOS", note: "16 equipos · eliminación directa" },
  { name: "CUARTOS", note: "8 equipos · 4 partidos" },
  { name: "SEMIFINALES", note: "4 equipos · 2 partidos" },
  { name: "PODIO", note: "3.er lugar + gran final · top 3 definido en cancha" }
];
...
const bracket = [
  ...
  },
  {
    title: "PODIO",
    matches: [
      { label: "3.er LUGAR", teams: ["PERDEDOR S1", "PERDEDOR S2"] },
      { label: "FINAL", teams: ["GANADOR S1", "GANADOR S2"] }
    ]
  },
  {
    title: "CAMPEÓN",
...
const rules = [
  ...
  {
    title: "Partidos",
    preview: "5 min por tiempo, 5 min de descanso y bloques operativos de 25 min."
  },
  {
    title: "Desempates",
    preview: "La tabla se ordena por puntos, diferencia de gol y goles a favor."
  },
  {
    title: "Fair Play",
    preview: "Respeto obligatorio a rivales, organización, público y equipos."
  },
  {
    title: "Sanciones",
    preview: "Advertencia, pérdida administrativa o descalificación según gravedad."
  }
];
...
const modalRules = [
  ...
  },
  {
    ...
    paragraphs: ["Si dos o más equipos terminan igualados en puntos, se aplicarán los criterios en este orden:"],
    bullets: [
...
const faq = [
  {
    ...
    answer: "El torneo está diseñado para 64 participantes, organizados en 32 equipos de dos jugadores."
  },
  {
    question: "¿Es 1 vs. 1 o 2 vs. 2?",
    answer: "La modalidad oficial de esta edición es 2 vs. 2. Cada pareja compite como un solo equipo durante todo el torneo."
  },
  {
    question: "¿Cómo funciona la fase de grupos?",
    answer: "Hay 4 grupos de 8 equipos. Cada equipo juega 2 partidos. Se otorgan 3 puntos por victoria, 1 por empate y 0 por derrota; clasifican los 4 mejores de cada grupo."
  },
  {
    question: "¿Cuántos equipos pasan a eliminación directa?",
    answer: "Avanzan 16 equipos: cuatro por cada grupo. Después se juegan octavos, cuartos, semifinales, tercer lugar y final."
  },
  {
    question: "¿A qué hora comienza y termina?",
    answer: "El kickoff oficial está previsto para las 9:30 a. m. y el cierre con premiación alrededor de las 5:30 p. m. El check-in se realiza antes del inicio."
  },
  {
    question: "¿Habrá descanso general para almorzar?",
    answer: "El cronograma de 8 horas no contempla una pausa general. Cada equipo tendrá descansos naturales mientras se disputan otros encuentros y deberá estar atento a sus llamados."
  },
  {
    question: "¿Cuánto dura cada partido?",
    answer: "Se configuran 5 minutos por tiempo y 5 minutos de entretiempo. Operativamente se reserva un bloque total de 25 minutos por encuentro, incluyendo hasta 10 minutos para rotación y preparación."
  },
  {
    question: "¿Cuántas estaciones habrá?",
    answer: "Habrá 3 estaciones, por lo que pueden jugarse hasta 3 partidos simultáneamente."
  },
  {
    question: "¿Cómo se define el tercer lugar?",
    answer: "Los dos equipos que pierdan las semifinales juegan un partido específico por el 3.er lugar antes de la gran final."
  },
  {
    question: "¿Dónde se realizará?",
    answer: "La sede indicada es la Universidad Gerardo Barrios, Centro Regional Usulután. El espacio exacto dentro del campus será comunicado por AECOMPU."
  },
  {
    question: "¿Debo llevar mi propio control?",
    answer: "AECOMPU comunicará antes del evento la política definitiva sobre controles propios y periféricos admitidos."
  },
  {
    question: "¿Dónde consulto todas las reglas?",
    answer: "En la sección REGLAS puedes abrir el reglamento completo con formato, puntuación, desempates, horarios, conducta, sanciones y procedimientos técnicos."
  }
];
...
function renderRoadmap() {
  ...
  container.innerHTML = tournamentStages
    .map((stage, index) => `
      <article class="road-step reveal fade-up" style="transition-delay:${Math.min(index * 70, 350)}ms">
        <div class="road-step__node"><span>${String(index + 1).padStart(2, "0")}</span></div>
        <div>
...
function renderMatches() {
  ...
  container.innerHTML = matches
    .map((match, index) => `
      <article class="match-card reveal fade-up" style="transition-delay:${Math.min(index * 80, 240)}ms">
        <div class="match-card__top">
          <span class="match-card__stage">${escapeHTML(match.stage)}</span>
          <span class="status ${getStatusClass(match.status)}">${escapeHTML(match.status)}</span>
        </div>
        <div class="match-card__body">
          ...
          <div class="match-card__footer">${escapeHTML(match.footer)}</div>
        </div>
      </article>
    `)
    .join("");
...
function renderBracket() {
  ...
  container.innerHTML = bracket
    .map((column, columnIndex) => {
      if (column.champion) {
        return `
          <section class="bracket-column bracket-column--champion">
            <div class="bracket-column__title">${escapeHTML(column.title)}</div>
            <div class="bracket-champion">
              <div>
                <span>CHAMPION</span>
                <strong>${escapeHTML(column.champion)}</strong>
              </div>
            </div>
          </section>
        `;
      ...
      const gapClass =
        columnIndex === 1 ? "bracket-gap-sm" :
        columnIndex === 2 ? "bracket-gap-lg" :
        columnIndex === 3 ? "bracket-gap-xl" : "";

      return `
        <section class="bracket-column ${gapClass}">
          <div class="bracket-column__title">${escapeHTML(column.title)}</div>
          ${column.matches.map((match) => `
            <div class="bracket-match ${match.label === "FINAL" ? "bracket-match--final" : ""}">
              ${match.label ? `<div class="bracket-match__label">${escapeHTML(match.label)}</div>` : ""}
              <div class="bracket-player"><span>${escapeHTML(resolveQualifiedSeed(match.teams[0]))}</span><strong>—</strong></div>
              <div class="bracket-player"><span>${escapeHTML(resolveQualifiedSeed(match.teams[1]))}</span><strong>—</strong></div>
            </div>
          `).join("")}
        </section>
      `;
    ...
    preview.innerHTML = rules
      .map((rule, index) => `
        <article class="rule-card">
          ...
          <h3>${escapeHTML(rule.title.toUpperCase())}</h3>
          <p>${escapeHTML(rule.preview)}</p>
        </article>
      `)
      .join("");
  }
  ...
  if (modalContent) {
    modalContent.innerHTML = `
      <div class="rules-summary">
        <strong>64 JUGADORES · 32 EQUIPOS · 2 VS 2 · 4 GRUPOS · TOP 16 · 3 ESTACIONES</strong>
        <span>Kickoff 9:30 A. M. · Cierre estimado 5:30 P. M.</span>
      </div>
      ${modalRules.map((rule) => `
        <section class="modal-rule">
          <h3>${escapeHTML(rule.title)}</h3>
          ${(rule.paragraphs || []).map((paragraph) => `<p>${escapeHTML(paragraph)}</p>`).join("")}
          ${rule.bullets ? `<ul>${rule.bullets.map((bullet) => `<li>${escapeHTML(bullet)}</li>`).join("")}</ul>` : ""}
        </section>
      `).join("")}
    `;
  }
  ...
  container.innerHTML = schedule
    .map((event, index) => `
      <article class="schedule-item reveal fade-up">
        <div class="schedule-item__num">${String(index + 1).padStart(2, "0")}</div>
...
function setupRegistration() {
  ...
  if (deadline) deadline.textContent = formatConfiguredDate(TOURNAMENT_CONFIG.registrationDeadline);
  ...
  if (button) {
    if (TOURNAMENT_CONFIG.registrationUrl === "#") {
      ...
      button.href = TOURNAMENT_CONFIG.registrationUrl;
      button.target = "_blank";
      button.rel = "noopener noreferrer";
    }
  }

  if (andradeLink) andradeLink.href = TOURNAMENT_CONFIG.andradeDevUrl || "#";
}

function setupCountdown() {
  const countdown = document.getElementById("countdown");
  const empty = document.getElementById("countdownEmpty");
  if (!countdown || !empty) return;

  if (!TOURNAMENT_CONFIG.tournamentDate) {
    countdown.hidden = true;
    empty.hidden = false;
    empty.textContent = `FECHA POR CONFIRMAR · KICKOFF ${TOURNAMENT_CONFIG.kickoff}`;
    return;
  }

  const target = new Date(TOURNAMENT_CONFIG.tournamentDate);
  ...
  const update = () => {
    ...
    if (distance <= 0) {
      countdown.hidden = true;
      empty.hidden = false;
      empty.textContent = "EL TORNEO HA COMENZADO";
      return false;
    }
  ...
  const observer = new IntersectionObserver((entries) => {
    entries.forEach((entry) => {
      ...
      const animate = (now) => {
        ...
        node.textContent = `${format === "02" ? String(value).padStart(2, "0") : value}${suffix}`;
...
function setupLogoFallback() {
  const image = document.getElementById("aecompuLogo");
  const placeholder = document.getElementById("aecompuLogoPlaceholder");
  ...
  if (TOURNAMENT_CONFIG.aecompuLogo) image.src = TOURNAMENT_CONFIG.aecompuLogo;
}
...
function getStatusClass(status) {
  ...
  if (["FINAL", "CIERRE"].includes(normalized)) return "status--final";
  if (["LIVE", "EN JUEGO", "KICKOFF", "LISTA"].includes(normalized)) return "status--live";
...
function formatConfiguredDate(value) {
  ...
  return new Intl.DateTimeFormat("es-SV", { day: "2-digit", month: "long", year: "numeric" }).format(date).toUpperCase();
}
